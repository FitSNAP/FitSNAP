...
class ElasticPropertiesFromLAMMPS:
    def __init__(self, snap, lammps_executable_path, lammps_script_filename, truth_values=False, existing_output_path=False):
        ####LOGAN NOTE: Okay, so the broad strokes idea is to switch this up so it becomes parallel friendly
        ...
        ##############: L.command(), L.commands_list(), and L.commands_string() can be used to turn all these strings, lists of strings, and multi-line strings into lammps commands
        ##############: L.extract_variable(), L.extract_fix(), etc. can be used to grab the output from this or similar workflows
        ##############: (only extract_variable needed for these)
        ##############: all the printing currently in the commands can be turned off

        ##############: the lammps_executable_path can be removed as it will just use the lammps that fitsnap also uses
        ##############: the lammps_script_filename can be removed as all the workflow will be internal to this class
    ...
    def run_lammps(self):
        L=lammps()
        #TODO: expose the finite deformation size and crystal structure type variables as class init settings
        calculation_init_list=["# NOTE: This script can be modified for different atomic structures, ",\
        ...
        #TODO get these exposed or make new function for snap.output that will return the strings instead of writing the output
        from fitsnap3lib.io.outputs.snap import _to_coeff_string
        from fitsnap3lib.io.outputs.snap import _to_param_string
        from fitsnap3lib.io.outputs.snap import _to_potential_file

        snapcoeff_string = _to_coeff_string(snap.config, snap.solver.fit)
        snapparam_string = _to_param_string(snap.config)
        snapmod_string = _to_potential_file(snap.config)
        
        calculation_potential_list = ["# NOTE: This script can be modified for different pair styles ",\
        ...
        calculation_displace_list = "# NOTE: This script should not need to be",\
        ...
        calculation_main_list = ["# Compute elastic constant tensor for a crystal",\
                                 "#",\
                                 ...
                                 "variable relaxed_lattice_x equal ${lx0}/3.0",\
                                 "variable relaxed_lattice_y equal ${ly0}/3.0",\
                                 "variable relaxed_lattice_z equal ${lz0}/3.0",\
                                 ...
                                 "",\
                                 "print "x lattice constant = ${relaxed_lattice_x}"",\
                                 "print "y lattice constant = ${relaxed_lattice_y}"",\
                                 "print "z lattice constant = ${relaxed_lattice_z}"",\]
...
def fit_and_cost(fs, fitobjects, costweights, additional_cost_functions=[],additional_cost_weights = [], verbose = False):
    ...
    ### TODO: need to reinitialize the cost function classes here with the new snap objects
    ...
    *When using MPI, a quick note on number of cores P: 
    ...
    while generation <= ngenerations and best_score > conv_thr and not conv_flag:
        ...
        if parallel_population and ncores > 1:
            ...
            par_additional_cost_functions = [deepcopy(cfunc) for cfunc in additional_cost_functions]
        else:
            par_fs = fs
            par_additional_cost_functions = additional_cost_functions
        ...
        for i, creature in enumerate(pop_list):           
            ...
            costi = fit_and_cost(par_fs, [a, b, new_w, fs_dict], [etot_weight,ftot_weight,stot_weight], verbose=verbose, \
                                 additional_cost_functions=par_additional_cost_functions, additional_cost_weights=additional_cost_weights)
