    ...
    class ARD(Solver):
        ...
        def perform_fit(self):
            ...
            def decorated_perform_fit():
                ...
                if directmethod:
                    reg = ARDRegression(max_iter=10, threshold_lambda=thresh, alpha_1=alval_big, alpha_2=alval_big,
                                        lambda_1=lmbval_small, lambda_2=lmbval_small, fit_intercept=False, verbose=self.config.debug)
                else:
                    reg = ARDRegression(max_iter=10,alpha_1=scap*ap, alpha_2=scap*ap, lambda_1=ap*scai,lambda_2=ap*scai,fit_intercept=False,threshold_lambda= 10**(int(np.abs(np.log10(ap)))+logcut), verbose=self.config.debug )
