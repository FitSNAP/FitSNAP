...
try:
    ...
    class JAX(Solver):
        ...
        def __init__(self, name):
            ...
            self.params = config.sections["JAX"].params
            self.learning_rate = config.sections["JAX"].learning_rate
        ...
        def inital_setup(self):
            if config.sections["JAX"].save_state_input is not None:
                self.load_pytorch()
            else:
                # Standardization step
                inv_std = 1 / np.std(pt.shared_arrays['a'].array, axis=0)
                mean_inv_std = jnp.array(np.mean(pt.shared_arrays['a'].array, axis=0) * inv_std)
                inv_std = jnp.array(inv_std * np.eye(len(inv_std)))
                self.params[0] = (inv_std, mean_inv_std)
            training = [not elem for elem in pt.fitsnap_dict['Testing']]
            self.truth_mean = np.mean(pt.shared_arrays['b'].array, axis=0)

            self.training_data = InRAMDatasetJAX(pt.shared_arrays['a'].array[training],
                                                 pt.shared_arrays['b'].array[training])
            self.training_loader = DataLoader(self.training_data,
                                              batch_size=config.sections["JAX"].batch_size,
                                              shuffle=False,
                                              collate_fn=jax_collate,
                                              num_workers=0)
            if config.sections["JAX"].opt_state_input is not None:
                with open(config.sections["JAX"].opt_state_input, 'rb') as fp:
                    self.opt_state = pickle.load(fp)
            else:
                self.opt_state = self.optimizer.init(self.params)
            self.targets = jnp.array(np.reshape(pt.shared_arrays['b'].array[pt.shared_arrays['a'].energy_index], (-1, 1)))
            self.num_atoms = jnp.array(np.reshape(pt.shared_arrays['number_of_atoms'].array, (-1, 1)))
            for i, ind in enumerate(pt.shared_arrays['number_of_atoms'].array):
                temp = [i]*ind
        ...
        def perform_fit(self):
            fit_start = time()
            self.inital_setup()
            for epoch in range(config.sections["JAX"].num_epochs):
                ...
                for batch in self.training_loader:
                    ...
                    self.params, self.opt_state = self.update(self.params,
                                                              ...
                                                              self.opt_state,
                                                              batch['nseg'])
                train_acc = accuracy(self.params, (pt.shared_arrays['a'].array-self.mean)/self.std, self.targets, self.indices, self.num_atoms)
                train_mae = mae(self.params, (pt.shared_arrays['a'].array-self.mean)/self.std, self.targets, self.indices, self.num_atoms)
            ...
            if config.sections["JAX"].output_style == 'pytorch':
                self.create_pytorch()
            if config.sections["JAX"].opt_state_output is not None:
                with open(config.sections["JAX"].opt_state_output, 'wb') as fp:
        ...
        def load_pytorch(self):
            ...
            pytorch_network = create_torch_network(config.sections["JAX"].layer_sizes)
            pytorch_model = FitTorch(pytorch_network, config.sections["CALCULATOR"].num_desc)
            pytorch_model.load_lammps_torch(config.sections["JAX"].save_state_input)
