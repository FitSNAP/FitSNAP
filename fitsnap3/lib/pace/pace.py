...
class CouplingCoeffs:
    ...
    def __init__(self, nus=None, lmax=14, wflag=False):
        ...
        if not wflag:
            pt.single_print('using default generalized Wigner 3j couplings')
            pt.single_print('generalized CG couplings have been removed')
    ...
    def find_cg(self):
        try:
            with open('%s/Clebsch_Gordan.pickle' % self.lib_path, 'rb') as handle:
                pt.single_print("Opening Clebsch Gordan Pickle")
                self.clebsch_gordan = pickle.load(handle)
                pt.single_print("Finished Opening Clebsch Gordan Pickle")
        except FileNotFoundError:
            pt.single_print("Generating your first pickled library of CG coefficients. This will take a few moments...")
            if pt.get_rank() == 0:
    ...
    def find_w3j(self):
        try:
            with open('%s/Wigner_3j.pickle' % self.lib_path, 'rb') as handle:
                pt.single_print("Opening Wigner Pickle")
                self.w3j = pickle.load(handle)
                pt.single_print("Finished Opening Wigner Pickle")
        except FileNotFoundError:
            pt.single_print(
