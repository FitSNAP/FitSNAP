...
class LammpsBase(Calculator):
    ...
    def preprocess_allocate(self, nconfigs: int):
        ...
        self.pt.create_shared_array('number_of_dgradrows', nconfigs, tm=self.config.sections["SOLVER"].multinode)
        # number of neighbors per config, organized like the other shared arrays in calculator.py
        self.pt.create_shared_array('number_of_neighs', nconfigs, tm=self.config.sections["SOLVER"].multinode)
