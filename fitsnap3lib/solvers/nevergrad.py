...
class NGOpt(Solver):
    ...
    def _transform_old(self, x):
        """Forward transform: full physical array → transformed optimization space."""
        return np.array([
            x[0],                          # GEN.bond_softness
            x[1],                          # ATM.N.r_s
            x[2],                          # ATM.N.gamma
    ...
    def _inverse_transform_old(self, y):
    ...
    def _transform(self, x):
        """Forward transform: full physical array → transformed optimization space."""
        # Scale parameters to [0,1] unit hypercube for better optimization
        normalized = np.zeros(9, dtype=np.float64)
        
        # Direct parameters (no constraints)
        normalized[0] = (x[0] - self.lower[0]) / (self.upper[0] - self.lower[0])  # GEN.bond_softness
        normalized[1] = (x[1] - self.lower[1]) / (self.upper[1] - self.lower[1])  # ATM.N.r_s
        normalized[2] = (x[2] - self.lower[2]) / (self.upper[2] - self.lower[2])  # ATM.N.gamma
        normalized[3] = (x[3] - self.lower[3]) / (self.upper[3] - self.lower[3])  # ATM.N.chi
        normalized[8] = (x[5] - self.lower[8]) / (self.upper[8] - self.lower[8])  # ATM.O.r_s
        normalized[5] = (x[6] - self.lower[5]) / (self.upper[5] - self.lower[5])  # ATM.O.gamma
        
        # Constrained parameters
        # For eta > 8.13*gamma, we use a positive offset
        eta_n_min = 8.13 * x[2]  # Minimum value for N.eta
        normalized[4] = (x[4] - eta_n_min) / (self.upper[4] - self.lower[4])  # Normalized excess of N.eta
        
        eta_o_min = 8.13 * x[6]  # Minimum value for O.eta
        normalized[6] = (x[8] - eta_o_min) / (self.upper[6] - self.lower[6])  # Normalized excess of O.eta
        
        # For chi(O) > chi(N), we use a positive offset
        chi_o_min = x[3]  # Minimum value for O.chi 
        normalized[7] = (x[7] - chi_o_min) / (self.upper[7] - self.lower[7])  # Normalized excess of O.chi
    ...
    def inverse_transform(self, y):
        ...
        # Direct parameters
        x[0] = y[0] * (self.upper[0] - self.lower[0]) + self.lower[0]  # GEN.bond_softness
        x[1] = y[1] * (self.upper[1] - self.lower[1]) + self.lower[1]  # ATM.N.r_s
        x[2] = y[2] * (self.upper[2] - self.lower[2]) + self.lower[2]  # ATM.N.gamma
        x[3] = y[3] * (self.upper[3] - self.lower[3]) + self.lower[3]  # ATM.N.chi
        x[5] = y[8] * (self.upper[8] - self.lower[8]) + self.lower[8]  # ATM.O.r_s
        x[6] = y[5] * (self.upper[5] - self.lower[5]) + self.lower[5]  # ATM.O.gamma
        
        # Apply constraints
        # N.eta = 8.13*gamma + delta, where delta ≥ 0
        eta_n_min = 8.13 * x[2]
        eta_n_delta = y[4] * (self.upper[4] - self.lower[4]) + self.lower[4]
        x[4] = eta_n_min + max(0, eta_n_delta)  # Ensure delta is positive
        
        # O.eta = 8.13*gamma + delta, where delta ≥ 0
        eta_o_min = 8.13 * x[6]
        eta_o_delta = y[6] * (self.upper[6] - self.lower[6]) + self.lower[6]
        x[8] = eta_o_min + max(0, eta_o_delta)  # Ensure delta is positive
        
        # O.chi = N.chi + delta, where delta ≥ 0
        chi_o_min = x[3]
        chi_o_delta = y[7] * (self.upper[7] - self.lower[7]) + self.lower[7]
        x[7] = chi_o_min + max(0, chi_o_delta)  # Ensure delta is positive
        
        # Final bounds check for safety
        x = np.clip(x, 
                   [self.lower[0], self.lower[1], self.lower[2], self.lower[3], 0, self.lower[8], self.lower[5], 0, 0],
                   [self.upper[0], self.upper[1], self.upper[2], self.upper[3], 20.0, self.upper[8], self.upper[5], 10.0, 20.0])
        
        np.set_printoptions(precision=4, suppress=True, linewidth=2000)
        
        if np.any(np.isnan(x)) or np.any(np.isinf(x)):
            print("*** inverse_transform WARNING: NaN or inf detected in x")
    ...
    def _log(self, optimizer, candidate, loss):
        ...
        if optimizer.num_ask==1 or (now - self._last_log_time) >= 1*60:
            ...
            best_x = self._inverse_transform(best._value.copy())
